...
void Camera::initialize()
{
    ...
    for (Id = 0; Id < list->num; Id++) {
        ...
        if (!camera) {
            printf("Failed to initialize camera with guid %llx", list->ids[Id].guid);
            continue;
        }
...
void Camera::getImage()
{
    ...
    triclopsSetSurfaceValidationDifference(triclops, 0.5);
      ...
      //  for(int i=0; i<repeatScan; i++) {
        ...
        error = triclopsGetImage16(triclops, TriImg16_DISPARITY, TriCam_REFERENCE, &depthImage);
        ...
        // retrieve the depth image from the context
        error = triclopsGetImage(triclops, TriImg_RECTIFIED, TriCam_LEFT, &rectifiedImage);
        _HANDLE_TRICLOPS_ERROR("triclopsGetImage()", error);

        // save the rectified image
       if(sprintf(filenameRectified, "%s%d%s", "../outputs/rectified/rectified-", v, ".pgm") == -1)
        {
