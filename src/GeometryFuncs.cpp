...
list<PointXY> findPathAStar(const PointXY& startPt, const PointXY& goalPt, vector<Surface> surfaces) {
    Node *start = new Node(surfaces, startPt.getX(), startPt.getY(), 0);
    Node *goal = new Node(surfaces, goalPt.getX(), goalPt.getY(), 0);
    list<Node*> closedSet;
    list<Node*> openSet;
    openSet.push_back(start);
    Node *currentNode = start;
    bool finished = false;
    bool currentNodeChanged = true;

    //Explore possible paths
    while (!finished && currentNodeChanged) {
        currentNodeChanged = false;

        vector<Node*> neighbours = currentNode->findNeighbours(surfaces);
        double minF = -1;
        int indexMinF = -1;
        for (unsigned int i = 0; i < neighbours.size(); i++) {
            if (neighbours[i]->isAccessible() && !neighbours[i]->isClosed(closedSet)) {
                //If too slow, check if node is closed in findNeighbours 
                //(all neighbours are close so don't need to search the entire list)
        ...
        if (indexMinF != -1) {
            ...
            currentNode = neighbours[indexMinF];
    ...
    if (!finished) {
        cout << "A* algorithm failed to find a way" << endl;
    ...
    while (currentNode->point != start->point) {
        reversePath.push_back(currentNode->point);
